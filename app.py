...
# Inisialisasi lock global untuk mengelola stream kamera aktif
camera_lock = threading.Lock() # Dipindahkan ke sini untuk memastikan definisi awal
...
# Coba ambil jalur Tesseract dari variabel lingkungan 'TESSERACT_PATH'.
# Ini adalah metode yang paling fleksibel untuk berbagai OS.
tesseract_path_env = os.getenv('TESSERACT_PATH')

if tesseract_path_env:
    pytesseract.pytesseract.tesseract_cmd = tesseract_path_env
    print(f"Menggunakan Tesseract dari variabel lingkungan: {tesseract_path_env}")
else:
    # Fallback untuk Windows jika variabel lingkungan tidak diatur.
    # Jika Tesseract tidak diinstal di jalur default ini, atau Anda di OS lain,
    # Anda harus mengatur variabel lingkungan 'TESSERACT_PATH' secara manual.
    default_windows_path = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
    if sys.platform == "win32" and os.path.exists(default_windows_path):
        pytesseract.pytesseract.tesseract_cmd = default_windows_path
        print(f"Menggunakan Tesseract dari jalur default Windows: {default_windows_path}")
    else:
        # Coba 'tesseract' jika ada di PATH sistem (umum di Linux/macOS setelah instalasi)
        pytesseract.pytesseract.tesseract_cmd = 'tesseract'
        print("Variabel lingkungan 'TESSERACT_PATH' tidak diatur atau jalur default Windows tidak ditemukan.")
        print("Mencoba menggunakan 'tesseract' dari PATH sistem Anda.")
        print("Jika terjadi error 'TesseractNotFoundError', pastikan Tesseract terinstal dan")
        print("jalurnya ditambahkan ke PATH sistem Anda, atau atur variabel lingkungan 'TESSERACT_PATH'.")
...
# Rute open_screenshots_folder telah dihapus karena tidak sesuai untuk deployment web.
# Sebagai gantinya, screenshot akan diakses langsung melalui rute /captured_plates/<filename>
# dan ditampilkan di halaman log.
